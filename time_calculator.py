def add_time(start, duration,day_of_week = False):
  day_of_the_week_index = {"monday": 0, "tuesday": 1, "wednesday": 2, "thursday": 3, "friday": 4, "saturday": 5, "sunday": 6}
  ...
  if(am_or_pm == "PM" and start_hours + (duration_hours % 12)   >= 12):
    amount_of_days += 1
  am_or_pm = am_and_pm_flips[am_or_pm] if   amount_of_am_pm_flips % 2 == 1 else am_or_pm

  return_time = str(end_hours) + ":" + str(end_minutes) + " " +   am_or_pm
  if(day_of_week):
    day_of_week = day_of_week.lower()
    index = int((day_of_the_week_index[day_of_week]) + amount_of_days) % 7
    new_day = day_of_the_week_array[index]
    return_time += ", " + new_day

  if(amount_of_days == 1):
    return_time = return_time + " " + "(next day)"
  elif (amount_of_days > 1):
    return_time = return_time + " (" + str(amount_of_days) + " days later)"    
